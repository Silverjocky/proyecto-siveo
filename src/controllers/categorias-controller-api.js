...
categoriasAPI.updateCategoria = async (req=request,res=request,next) =>{
    try {
        ...
        if(id==undefined || descripcion==undefined || precio == undefined){
            ...
            const conexion = await miConexion();
            const resultado = await conexion.query('UPDATE categoria SET descripcion = ?, precio = ? WHERE id = ?',[descripcion,precio,id]);
            if(resultado[0].affectedRows>0){
                res.status(200).json({
                    estado: 1,
...
categoriasAPI.agregarCategoria = async(req=request,res=request,next) =>{
    try {
        ...
        if( descripcion == undefined || precio == undefined){
            ...
            const conexion = await miConexion();
            const resultado = await conexion.query('INSERT INTO categoria(descripcion,precio) values(?,?)',[descripcion,precio]);
            if(resultado[0].affectedRows>0){
                res.status(201).json({
                    estado: 1,
